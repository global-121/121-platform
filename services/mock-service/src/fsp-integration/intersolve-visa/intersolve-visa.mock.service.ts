...
export class IntersolveVisaMockService {
  ...
  public unloadBalanceCardMock(): IntersolveLoadResponseDto {
    return {
      data: {
        success: true,
        errors: [],
  ...
  public getWalletMock(tokenCode: string): IntersolveGetWalletResponseDto {
  ...
  public activateWalletMock(tokenCode: string): IntersolveGetWalletResponseDto {
    const response = new IntersolveGetWalletResponseDto();
    response.status = 200;
    response.data = {
      success: true,
