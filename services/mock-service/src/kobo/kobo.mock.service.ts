...
export class KoboMockService {
  public getAssetDeployment(uid_asset: string): KoboAssetDeployment {
    if (uid_asset === KoboMockAssetUids.bodyThatTriggersErrors) {
      return {
    ...
    if (uid_asset === KoboMockAssetUids.notFound) {
    ...
    if (uid_asset === KoboMockAssetUids.happyFlowWithChanges) {
      const happyFlowFromDefinitionWithChanges = happyFlowFromDefinition;

      happyFlowFromDefinitionWithChanges.version_id =
        KoboMockAssetUids.happyFlowWithChanges;
      happyFlowFromDefinitionWithChanges.asset.version_id =
        KoboMockAssetUids.happyFlowWithChanges;

      happyFlowFromDefinitionWithChanges.asset.summary.languages.push(
        'French (fr)',
      );

      happyFlowFromDefinitionWithChanges.asset.content.survey.push({
        name: 'newAttribute',
        type: 'text',
        $kuid: 'newKuid123',
        label: [
      ...
      const attributeToUpdate =
        happyFlowFromDefinitionWithChanges.asset.content.survey.find(
      ...
      happyFlowFromDefinitionWithChanges.asset.content.survey =
        happyFlowFromDefinitionWithChanges.asset.content.survey.filter(
          (item) => item.name !== 'How_are_you_today_select_one',
        );

      return happyFlowFromDefinitionWithChanges;
    }

    // Default: happy flow asset deployment
