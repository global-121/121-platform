...
export class LoadTestService {
        ...
        new Promise<void>((resolve) => {
          setTimeout(async () => {
            ...
            for (let j = 0; j < 5; j++) {
              ...
              console.log("🚀 ~ file: load-test.service.ts:28 ~ LoadTestService ~ setTimeout ~ referenceIdNumeric:", referenceIdNumeric)
  ...
  public async importRegistration(
    ...
    const result = lastValueFrom(this.httpService.post(url, body, { headers: headers}))
  }

  public async getHeader(): Promise<any> {
    const url = `${this.baseUrl}/users/login`;
    const loginResult = await this.httpService.post(url, {
