  ...
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `SELECT setval('intersolve_issue_voucher_request_id_seq', (SELECT MAX(id) FROM intersolve_issue_voucher_request)+ 1)`,
    );
    await queryRunner.query(
      `SELECT setval('intersolve_voucher_id_seq', (SELECT MAX(id) FROM intersolve_voucher)+ 1)`,
    );
    await queryRunner.query(
      `SELECT setval('intersolve_voucher_instruction_id_seq', (SELECT MAX(id) FROM intersolve_voucher_instruction)+ 1)`,
    );
  }
