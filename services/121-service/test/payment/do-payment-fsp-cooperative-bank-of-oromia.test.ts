...
describe('Do payment with FSP: CooperativeBankOfOromia', () => {
  ...
  it('should successfully initiate a payment', async () => {
    ...
    const registrationAirtelInitiatePayment = {
      ...registrationAirtel,
      referenceId: 'cooperative-bank-of-oromia-initiate-payment',
    };
  ...
  it('should successfully do a payment', async () => {
    ...
    const registrationAirtelSuccessTransaction = {
      ...registrationAirtel,
      referenceId: 'cooperative-bank-of-oromia-success-transaction',
    };
  ...
  it('should correctly handle duplicate CooperativeBankOfOromia transactionId', async () => {
    // Arrange
    const registrationAirtelDuplicateTransactionId = {
      ...registrationAirtel,
      phoneNumber: '260000000001',
      referenceId: 'cooperative-bank-of-oromia-duplicate-transaction-id',
    };
    const paymentReferenceIds = [
  ...
  it('should create a transaction with status error when we do a payment with an invalid phonenumber', async () => {
    ...
    const registrationAirtelDuplicateTransactionId = {
      ...
      referenceId: 'cooperative-bank-of-oromia-invalid-phonenumber',
    };
    const paymentReferenceIds = [
      registrationAirtelDuplicateTransactionId.referenceId,
    ];
  ...
  it('should create a transaction with status waiting when we get an Ambiguous error from CooperativeBankOfOromia', async () => {
    // Arrange
    const registrationAirtelAmbiguousError = {
      ...registrationAirtel,
      phoneNumber: '260000000003',
      referenceId: 'cooperative-bank-of-oromia-ambiguous-error',
    };
    ...
    expect(transaction.errorMessage).toMatch(
      /^CooperativeBankOfOromia Error: Please use the CooperativeBankOfOromia Mobiquity portal to find out the status of the transaction. CooperativeBankOfOromia transaction id: [a-z0-9]{64} - Status: Ambiguous/,
    );
  });
});
