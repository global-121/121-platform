...
- Include the (auto-generated) changes in `swagger.json` in commits together with changes to any `*.controller.ts`-file.
  - This `swagger.json`-file is generated by running the application in development-mode
...
docker exec 121-service  npm run test:integration:base delete-program.test.ts # specific test file
```
