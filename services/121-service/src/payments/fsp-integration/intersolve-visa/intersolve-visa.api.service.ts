...
export class IntersolveVisaApiService {
  ...
  public async createCustomer(
    ...
    const createCustomerRequestDto: CreateCustomerRequestDto = {
      ...
      contactInfo: {
        addresses: [
          {
            type: 'HOME',
            addressLine1: createCustomerDto.street,
    ...
    // Send the request
    let createCustomerResponseDto;
    if (process.env.MOCK_INTERSOLVE) {
      createCustomerResponseDto =
        await this.intersolveVisaApiMockService.createCustomerMock(
          createCustomerRequestDto,
        );
    } else {
  ...
  // TODO: Remove this function, replaced by this.issueToken()
  ...
  public async issueToken(
    ...
    // Send the request
    let issueTokenResponseDto;
    if (process.env.MOCK_INTERSOLVE) {
      issueTokenResponseDto =
        await this.intersolveVisaApiMockService.createWalletMock(
          issueTokenRequestDto.reference,
        );
    } else {
      const authToken = await this.getAuthenticationToken();
      const apiPath = process.env.INTERSOLVE_VISA_PROD
    ...
    const issueTokenResultDto: IssueTokenResultDto = {
      ...
      blocked: issueTokenResponseDto.data.data.token.blocked,
  ...
  public async getToken(tokenCode: string): Promise<GetTokenResultDto> {
    // Send the request
    let getTokenResponseDto: GetTokenResponseDto;
    if (process.env.MOCK_INTERSOLVE) {
      getTokenResponseDto =
        await this.intersolveVisaApiMockService.getWalletMock(tokenCode);
    } else {
      const authToken = await this.getAuthenticationToken();
      const apiPath = process.env.INTERSOLVE_VISA_PROD
        ? 'pointofsale-payments'
      ...
      getTokenResponseDto = await this.httpService.get<GetTokenResponseDto>(
        url,
        headers,
      );
    }
    ...
    if (tokenData?.balances) {
      balance = tokenData.balances.find(
  ...
  // TODO: REFACTOR: Use a DTO for input params? That would align with issueToken() and createCustomer(). But there are only 2 parameters. I am in doubt.
  public async registerHolder(
    ...
    let registerHolderResponse;
    // Send the request
    if (process.env.MOCK_INTERSOLVE) {
      // TODO: REFACTOR: Mock function:
      registerHolderResponse =
        await this.intersolveVisaApiMockService.linkCustomerToWalletMock(
          tokenCode,
        );
    } else {
      const authToken = await this.getAuthenticationToken();
      ...
      registerHolderResponse = await this.httpService.post<any>(
        url,
  ...
  // TODO: Remove this function, replaced by this.registerHolder().
  ...
  // TODO: Remove this function, replaced by this.createPhysicalCard().
  public async createDebitCard(
    tokenCode: string | null,
    payload: CreatePhysicalCardRequestDto,
  ): Promise<any> {
    const authToken = await this.getAuthenticationToken();
  ...
  public async createPhysicalCard(
    ...
    createPhysicalCardRequestDto.cardAddress = {
      address1:
        `${createPhysicalCardDto.street} ${createPhysicalCardDto.houseNumber} ${createPhysicalCardDto.houseNumberAddition}`.trim(),
      city: createPhysicalCardDto.city,
      country: 'NLD',
      postalCode: createPhysicalCardDto.postalCode,
    };
    ...
    let createPhysicalCardResponse;
    // Send the request
    if (process.env.MOCK_INTERSOLVE) {
      createPhysicalCardResponse =
        await this.intersolveVisaApiMockService.createDebitCardMock(
          createPhysicalCardDto.tokenCode,
        );
    } else {
      const authToken = await this.getAuthenticationToken();
      ...
      // On success this returns a 200 with a body containing correlationId
      createPhysicalCardResponse = await this.httpService.post<void>(
        url,
  ...
  private convertResponseErrorsToMessage(
    errorsInResponseDto: ErrorsInResponseDto[],
  ): string {
    if (
      !errorsInResponseDto ||
      ...
      return; //TODO: Does this return null? Should it return an empty string?
    }
    let message = '';
