...
describe('PaymentEventsMapper', () => {
  ...
  const createMockAttributeEventEntity = (
    key: PaymentEventAttributeKeyEnum,
    value: string,
  ...
  describe('mapToPaymentEventsDto', () => {
    it('should map payment event entities to return DTOs with user information', () => {
      ...
      const noteAttribute = createMockAttributeEventEntity(
        PaymentEventAttributeKeyEnum.note,
        testNote,
      );
      const paymentEvents = [
        createMockPaymentEventEntity(1, PaymentEventEnum.created, mockUser),
        createMockPaymentEventEntity(2, PaymentEventEnum.note, mockUser, [
          noteAttribute,
        ]),
      ];
      ...
      // Assert
      expect(meta).toEqual({
        availableTypes: [PaymentEventEnum.created, PaymentEventEnum.note],
        count: {
          [PaymentEventEnum.created]: 1,
          [PaymentEventEnum.note]: 1,
        },
        total: 2,
      });
      ...
      expect(data[1]).toEqual({
        ...
        attributes: {
          [PaymentEventAttributeKeyEnum.note]: testNote,
        },
      });
    });
    ...
    it('should handle events without user information', () => {
      // Arrange
      const paymentEvents = [
        createMockPaymentEventEntity(1, PaymentEventEnum.created, undefined),
      ];
      ...
      // Assert
      expect(meta).toEqual({
        availableTypes: [PaymentEventEnum.created],
        count: {
          [PaymentEventEnum.created]: 1,
        },
        total: 1,
