...
@Processor(TransactionJobQueueNames.airtel)
export class TransactionJobProcessorAirtel {
  constructor(
  ...
  async handleAirtelTransactionJob(job: Job): Promise<void> {
    try {
      await this.transactionJobProcessorsService.processAirtelTransactionJob(
        job.data,
      );
    } catch (error) {
      console.log(error);
