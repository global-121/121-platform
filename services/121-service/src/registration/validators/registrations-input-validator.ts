...
export class RegistrationsInputValidator {
      ...
      await Promise.all(
        program.programRegistrationAttributes.map(async (att) => {
          ...
          if (att.type === RegistrationAttributeTypes.tel) {
            // return on bulk (pre queue); this is a slow validation, so we only do this post queue
  ...
  private validatePreferredLanguage({
    ...
    preferredLanguage: RegistrationPreferredLanguageEnum | undefined;
  } {
  ...
  ): RegistrationPreferredLanguageEnum | undefined {
    if (!preferredLanguage) {
      return RegistrationPreferredLanguageEnum.en;
    }
