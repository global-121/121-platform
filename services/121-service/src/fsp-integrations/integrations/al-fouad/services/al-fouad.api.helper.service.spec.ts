...
jest.mock('@121-service/src/env', () => ({
  env: {
    AL_FOUAD_MODE: 'MOCK', // String literal to avoid using FspMode before jest.mock is initialized
    MOCK_SERVICE_URL: 'http://mock-service:3001',
    AL_FOUAD_API_URL: 'https://alfouad.example.org',
  },
}));
...
describe('AlFouadApiHelperService', () => {
  ...
  beforeEach(async () => {
    ...
    (env as any).AL_FOUAD_MODE = FspMode.mock;
    (env as any).MOCK_SERVICE_URL = 'http://mock-service:3001';
    (env as any).AL_FOUAD_API_URL = 'https://alfouad.example.org';
  });

  describe('Resolving the base URL', () => {
    it('should return the mock service URL when AL_FOUAD_MODE is mock', () => {
      // Arrange
      (env as any).AL_FOUAD_MODE = FspMode.mock;
      ...
      expect(result.toString()).toBe(
        'http://mock-service:3001/api/fsp/al-fouad/',
      );
    });

    it('should return the Al Fouad API URL when AL_FOUAD_MODE is external', () => {
      // Arrange
      (env as any).AL_FOUAD_MODE = FspMode.external;
      (env as any).AL_FOUAD_API_URL = 'https://alfouad.example.org';
