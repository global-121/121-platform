...
export class IntersolveVisaService {
  ...
  public async closeCardOrThrow({
    ...
    const authToken =
      await this.intersolveVisaApiService.getAuthenticationToken();
    // TODO: Remove console.log once Intersolve API is stable
    console.log(
      `[Close Card CURL - Change Status]\n` +
        `curl -X 'PATCH' \\\n` +
        `  '${env.INTERSOLVE_VISA_API_URL}/payment-instrument-payment/v1/tokens/${childTokenCode}/change-status/${IntersolveVisaCardStatus.CardClosed}' \\\n` +
        `  -H 'Tenant-ID: ${env.INTERSOLVE_VISA_TENANT_ID}' \\\n` +
