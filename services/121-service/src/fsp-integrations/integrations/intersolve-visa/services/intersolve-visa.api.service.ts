...
export class IntersolveVisaApiService {
  ...
  public async closeCard({ tokenCode }: { tokenCode: string }): Promise<void> {
    await this.intersolveApiRequest<void>({
      ...
      method: 'PATCH',
      apiPath: 'payment-instrument-payment',
      endpoint: `tokens/${tokenCode}/change-status/${IntersolveVisaCardStatus.CardClosed}`,
    });
  }
  ...
  >({
    ...
    if (error) {
      ...
      console.log(
        `[Intersolve API] ${method} ${endpoint} - ERROR:`,
      ...
      // TODO: Remove console.log once Intersolve API is stable
      console.log(
        `[Intersolve API] ${method} ${endpoint} - OK:`,
        JSON.stringify(
          (success as BaseResponseIntersolveApiDto | undefined)?.data,
        ),
      );
