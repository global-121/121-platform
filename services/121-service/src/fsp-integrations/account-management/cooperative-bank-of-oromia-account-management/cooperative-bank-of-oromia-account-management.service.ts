...
export class CooperativeBankOfOromiaAccountManagementService {
  ...
  public async retrieveAndUpsertAccountValidations(): Promise<number> {
    const programs = await this.programRepository.getAllProgramIdsWithFsp(
      Fsps.cooperativeBankOfOromia,
    );
    let totalAccountValidations = 0;
    for (const programId of programs) {
      const accountValidationsPerProgram =
        await this.retrieveAndUpsertAccountValidationsForProgram({ programId });
      totalAccountValidations += accountValidationsPerProgram;
    }
    return totalAccountValidations;
  }
  ...
  private async storeAccountValidationResult(
    ...
    const namesMatch = this.doTheNamesMatch({
      registrationName: registration[FspAttributes.fullName]!,
      bankName: accountInformation.cooperativeBankOfOromiaName,
    });
