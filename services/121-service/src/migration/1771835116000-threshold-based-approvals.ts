...
export class ThresholdBasedApprovals1771835116000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Add programApprovalThresholdId and approverOrder columns to program_aidworker_assignment table
    await queryRunner.query(
      `ALTER TABLE "121-service"."program_aidworker_assignment" ADD "programApprovalThresholdId" integer NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "121-service"."program_aidworker_assignment" ADD "approverOrder" integer NULL`,
    );

    // Data migration: Create default approval thresholds for programs with approvers
    // For each program that has approvers, create a threshold with amount 0 and approval level 1
    ...
    for (const row of programsWithApprovers) {
      ...
      const thresholdResult = await queryRunner.query(
        `INSERT INTO "121-service"."program_approval_threshold" ("programId", "thresholdAmount", "approvalLevel", "created", "updated")
