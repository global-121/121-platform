...
import { registrationWesteros1 } from '@121-service/test/registrations/pagination/pagination-data';
...
test('User can view the registration data of registration that has all data types', async ({
  ...
  registrationId = await getRegistrationIdByReferenceId({
    programId: projectId,
    referenceId: registrationWesteros1.referenceId,
    accessToken,
  });
...
test('User can view the registration data of registration that has only the required data', async ({
  ...
  const registrationWithOnlyRequiredData = {
    ...
    programFinancialServiceProviderConfigurationName:
      registrationWesteros1.programFinancialServiceProviderConfigurationName,
    phoneNumber: registrationWesteros1.phoneNumber,
    house: registrationWesteros1.house,
    preferredLanguage: registrationWesteros1.preferredLanguage,
  };
  await seedIncludedRegistrations(
