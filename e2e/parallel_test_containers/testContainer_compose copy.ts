...
(function () {
  ...
  function mySpawn() {
    ...
    var result = oldSpawn.apply(arguments);
    return result;
  }
...
async function startPortal(servicePort: number): Promise<number> {
  ...
  const port = await checkPort(8089);
  ...
  // const command = `start --prefix  interfaces/Portal/ -- --port  ${port}`;
  // console.log(command);
  // const options = {
  //   cwd: rootdir,
  ...
  try {
    const command = 'npm';
    const args = ['start', '--prefix', 'interfaces/Portal/', '--', '--port', '8089'];
    const options = {
      stdio: 'inherit',
      cwd: 'c:\\work\\code\\121-platform',
      env: { ...process.env, NG_URL_121_SERVICE_API: 'http://localhost:3000/api' } // Include existing environment variables
    };

    await runNpmCommand(command, args, options);
