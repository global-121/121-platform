...
export default function () {
  // REFACTOR: this test requires the same setup as getProjectWithManyAttributes.js. Move setup code to shared place.
  // reset db
  ...
  // add 50 project registration attributes to generate a bigger load
  for (let i = 1; i <= 50; i++) {
    const attributeName = `attribute${i}`;
    const projectRegistrationAttributes =
      projectsPage.createProjectRegistrationAttribute(projectId, attributeName);
    registrationVisa[attributeName] = 'bla';

    checkAndFail(projectRegistrationAttributes, {
      'Project registration attributes added successfully status was 201': (
        r,
      ) => {
        if (r.status != 201) {
          console.log(r.body);
  ...
  // get project by id and validate load time is less than 200ms
  const project = projectsPage.getProjectById(projectId);
  checkAndFail(project, {
    'Projectme loaded successfully status was 200': (r) => r.status == 200,
    'Projectme load time is less than 200ms': (r) => {
      if (r.timings.duration >= 200) {
        console.log(`Projectme time was ${r.timings.duration}ms`);
      }
      return r.timings.duration < 200;
    },
  });
  ...
  const responseIncluded = projectsPage.updateRegistrationStatusAndLog(
    projectId,
    'included',
  );
