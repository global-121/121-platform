...
for file in "${test_files[@]}"; do
  ...
  (cd ../services || return; docker --log-level 'warn' compose -f docker-compose.yml up -d --quiet-pull --wait --wait-timeout 300)
  ...
  if [[ $(jq '.metrics.checks.fails' summary.json || true) -gt 0 ]]; then
      echo "Test failed: ${file}"
      failed_tests+=("${file}")
  fi
  echo "Stopping services"
  (cd ../services || return ; docker compose -f docker-compose.yml down)
done
