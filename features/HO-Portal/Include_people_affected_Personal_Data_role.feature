...
Feature: Include people affected by "personal data" role (extension of Manage_people_affected.feature)
  ...
  Scenario: Confirm "include for program" action
    ...
    And the "rejected" datetime remains visible, if the PA was "rejected" before
    And the "inclusion ended" datetime remains visible, if the PA had status "inclusion ended" before
