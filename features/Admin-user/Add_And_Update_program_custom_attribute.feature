...
Feature: Add and Update Program Custom Attribute
  ...
  Scenario: Successfully add custom attributes of program
    ...
    And calls the "POST /programs/{programId}/custom-attributes" endpoint
  ...
  Scenario: Successfully update custom attributes of program
    Given the "programId" and "attributeId" are provided
    Given the provided combination of "name" and "programId" does not already exist in the database
    When the user fills in the "name" and a subset of other attributes "type", "label" and "phases"
    And calls the "PATCH /programs/{programId}/custom-attributes/{attributeId}" endpoint
