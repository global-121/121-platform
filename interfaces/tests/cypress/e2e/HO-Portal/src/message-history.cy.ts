...
describe("Check message history", () => {
  ...
  it('Send 1 whatsapp message', function () {
    ...
    cy.fixture("message-history").then((fixture) => {
      ...
      const label = portalEn.page.program['program-people-affected']['message-history-popup']['content-type']['generic-templated']
      cy.fixture("registration-nlrc").then((registration) => {
        checkMessageHistoryPopup(registration, label, messageText, 'whatsapp', MessageStatus.delivered)
      })
    });
  });
  ...
  it('Send 1 sms message', function () {
    cy.fixture("registration-nlrc-no-whatsapp").then((registrationNoWhatsapp) => {
      ...
      cy.fixture("message-history").then((fixture) => {
        ...
        // Check Message History Popup
        const customLabel = portalEn.page.program['program-people-affected']['message-history-popup']['content-type'].custom
        checkMessageHistoryPopup(registrationNoWhatsapp, customLabel, fixture.messageText, 'sms', MessageStatus.sent)
      });
    });
  });
  ...
  it('Send 1 failed sms message', function () {
    cy.fixture("registration-nlrc-no-whatsapp").then((registrationNoWhatsapp) => {
      ...
      cy.fixture("message-history").then((fixture) => {
        ...
        // Check Message History Popup
        const customLabel = portalEn.page.program['program-people-affected']['message-history-popup']['content-type'].custom
        checkMessageHistoryPopup(registrationNoWhatsapp, customLabel, fixture.messageText, 'sms', MessageStatus.failed)
      });
    });
  });
      ...
      cy.fixture("message-history").then((fixture) => {
        ...
        cy.getAllPeopleAffected(programId).then((response) => {
          for (const pa of response.body) {
            cy.editPaAttribute(programId, pa.referenceId, 'phoneNumber', '15005550001')
          }

          cy.setHoPortal();
          const interceptIdMessage2 = 'textmessage2'
          cy.intercept({ 'method': 'POST', url: '**/text-message' }).as(interceptIdMessage2)
          cy.sendBulkMessage(fixture.messageText)

          // // Check PA-table
          checkPATable(fixture, MessageStatus.failed, 'SMS', interceptIdMessage2)

          // Check Message History Popup
          const customLabel = portalEn.page.program['program-people-affected']['message-history-popup']['content-type'].custom
          checkMessageHistoryPopup(registrationNoWhatsapp, customLabel, fixture.messageText, 'sms', MessageStatus.failed, 0)
          cy.visit(fixture.url);
          checkMessageHistoryPopup(registrationNoWhatsapp, customLabel, fixture.messageText, 'sms', MessageStatus.sent, 1)
        });
      });
    });
  });
  ...
  const checkMessageHistoryPopup = (registration: any, customLabel: string, messageText: string, messageType: string, messageStatus: MessageStatus,nEntry?: number) => {
    ...
    // Set labels
    const messageHistoryEn = portalEn.page.program['program-people-affected']['message-history-popup']
    let typeLabel
    ...
    // Checks row
    if (isFinite(nEntry)) {
      cy.get('[data-cy="message-history-row"]').eq(nEntry).contains(customLabel)
      cy.get('[data-cy="message-history-row"]').eq(nEntry).contains(typeLabel)
      cy.get('[data-cy="message-history-row"]').eq(nEntry).contains(messageStatus, { matchCase: false })
      cy.get('[data-cy="message-history-accordion"]').eq(nEntry).contains(messageTextSub)
      cy.get('[data-cy="message-history-accordion"]').eq(nEntry).click()
      cy.get('[data-cy="message-history-accordion"]').eq(nEntry).contains(messageText.replace(/\n/g, ''))
    } else {
      cy.get('[data-cy="message-history-row"]').contains(customLabel)
      cy.get('[data-cy="message-history-row"]').contains(typeLabel)
      cy.get('[data-cy="message-history-row"]').contains(messageStatus, { matchCase: false })
      cy.get('[data-cy="message-history-accordion"]').contains(messageTextSub)
      cy.get('[data-cy="message-history-accordion"]').click()
      cy.get('[data-cy="message-history-accordion"]').contains(messageText.replace(/\n/g, ''))
    }
  }
});
