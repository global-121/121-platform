...
import { NoEligibleApproversMessageComponent } from '~/components/no-eligible-approvers-message/no-eligible-approvers-message.component';
...
export class ProgramSettingsPaymentApprovalPageComponent {
  ...
  readonly approvalThresholds = injectQuery(
    this.programApiService.getApprovalThresholds(this.programId),
  );
  ...
  reorderStepsOnBlur(): void {
    const sortedControls = [...this.additionalSteps.controls].sort((a, b) => {
      const aValue = a.controls.thresholdAmount.value;
      const bValue = b.controls.thresholdAmount.value;
    ...
    for (const control of sortedControls) {
      this.additionalSteps.push(control);
    }
