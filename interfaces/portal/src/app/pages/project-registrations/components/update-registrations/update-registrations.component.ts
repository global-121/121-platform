...
type DownloadCSVFormGroup =
  (typeof UpdateRegistrationsComponent)['prototype']['downloadCSVFormGroup'];
...
export class UpdateRegistrationsComponent {
  ...
  downloadCSVFormGroup = new FormGroup({
  ...
  downloadCSVFormFieldErrors = generateFieldErrors<DownloadCSVFormGroup>(
    this.downloadCSVFormGroup,
    {
  ...
  exportCSVForUpdateRegistrations() {
    this.downloadCSVFormGroup.markAllAsTouched();

    if (!this.downloadCSVFormGroup.valid) {
      return;
    }

    const selectedFields = [
      'referenceId', // this is always required
      ...this.downloadCSVFormGroup.getRawValue().fields,
    ];
