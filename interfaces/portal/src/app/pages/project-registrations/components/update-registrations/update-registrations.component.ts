...
export class UpdateRegistrationsComponent {
  ...
  readonly stepOneLabel = computed(() => {
    const registrationCount = this.actionData()?.count;

    if (registrationCount) {
      return $localize`1. Create and export a CSV for the ${registrationCount} selected registration(s), select the columns you want to update.`;
    }

    return $localize`1. Create and export a CSV template, select the columns you want to update.`;
  });
  ...
  exportCSVForUpdateRegistrations() {
    ...
    const query = this.actionData()?.query;

    if (!query) {
      // no registrations selected, so we just export a template
      this.downloadService.downloadStringArrayToCSV({
        filename: 'update-registrations',
        file: [selectedFields.join(',')],
      });
